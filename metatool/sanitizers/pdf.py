...
class PDFSanitizer:
    ...
    def _sanitize_to_temporary_output(
        ...
        with pikepdf.open(source_path) as pdf_document:
            changes = self._apply_profile(pdf_document, sanitization_options)
    ...
    def _apply_profile(
        ...
        changes = self._remove_document_information(pdf_document)
        changes.extend(self._remove_xmp_metadata(pdf_document))
        if profile_name == "author":
            changes.append(self._replace_author(pdf_document, sanitization_options.author))
        return changes
